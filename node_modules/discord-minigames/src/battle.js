...
async function createBattle(member, message) {
    ...
    if (!member.id || !message.author) return message.reply("Veuillez spécifier un membre avec lequel jouer!");
    if (member.id === message.author.id)
        return message.reply(
            "Tu ne peux pas jouer avec toi-même!"
        );
    if (member.user.bot) return message.reply("Les robots/bots ne peuvent pas jouer!");
    ...
    async function start(member, channel, playerOne, playerTwo, message) {
        await channel
            .send(
                `Salut ${member}! ${message.author} vous a défié à une bataille, acceptez-vous?`
            )
            .then(async msg => {
                await msg.react("✅");
                await msg.react("❎");
                ...
                no.on("collect", async collected => {
                    await msg.edit("On dirait que ce lache refuse votre défi :/");
                    await yes.stop();
                });
                yes.on("collect", async collected => {
                    if (
                        ...
                        return msg.edit("Vous êtes déjà dans une bataille!");
                    await no.stop();
                ...
                setTimeout(() => {
                    if (
                        ...
                        return msg.edit("Ils n'ont pas réagi à temps :/");
                    }
                }, 15000);
            });
    }
                ...
                if (msg.content.toLowerCase() === attacks[i]) {
                    ...
                    if (chance[attackChance] === "yes") {
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${attacks[i]}** pour faire \`${attackAmount}\` dégats ${nextUp}, il dispose maintenant de \`${playerTwoData.battleHealth}\` HP !`
                                )
                                .setColor("RED")
                                .setAuthor(
                                    `Attaque par: ${nowBattling.displayName}`,
                                    nowBattling.user.displayAvatarURL()
                                )
                                .setFooter(
                                    `Prochaine étape: ${nextUp.displayName}`,
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${attacks[i]}** contre ${nextUp}, mais cela a échoué, il dispose maintenant de \`${playerTwoData.battleHealth}\` HP!`
                                )
                                .setColor("RED")
                                .setAuthor(
                                    `Attaquer par: ${nowBattling.displayName}`,
                                    nowBattling.user.displayAvatarURL()
                                )
                                .setFooter(
                                    `Prochain joueur: ${nextUp.displayName}`,
                                    nextUp.user.displayAvatarURL()
                                )
                        )
                ...
                if (msg.content.toLowerCase() === heals[x]) {
                    ...
                    if (chance[healChance] === "yes") {
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${heals[x]}** et se soigne\`${healAmount}\` HP, il a maintenant  \`${playerOneData.battleHealth}\` HP!`
                                )
                                .setColor("GREEN")
                                .setAuthor(
                                    `Guérir par: ${nowBattling.displayName}`,
                                    nowBattling.user.displayAvatarURL()
                                )
                                .setFooter(
                                    `Prochain joueur: ${nextUp.displayName}`,
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${heals[x]}** mais cela a échoué, il dispose maintenant de \`${playerTwoData.battleHealth}\` HP!`
                                )
                                .setColor("GREEN")
                                .setAuthor(
                                    `Heal par: ${nowBattling.displayName}`,
                                    nowBattling.user.displayAvatarURL()
                                )
                                .setFooter(
                                    `Prochain joueur: ${nextUp.displayName}`,
                ...
                if (msg.content.toLowerCase() === attacks[i]) {
                    ...
                    if (chance[attackChance] === "yes") {
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${attacks[i]}** pour faire \`${attackAmount}\` de dégats à ${nextUp}, Il lui reste donc  \`${playerOneData.battleHealth}\` HP!`
                                )
                                .setColor("RED")
                                .setAuthor(
                                    `Attaquer par: ${nowBattling.displayName}`,
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${attacks[i]}** contre ${nextUp}, mais cela a échoué, il a toujours \`${playerTwoData.battleHealth}\` HP!`
                                )
                                .setColor("RED")
                                .setAuthor(
                                    `Attaquer par: ${nowBattling.displayName}`,
                ...
                if (msg.content.toLowerCase() === heals[x]) {
                    ...
                    if (chance[healChance] === "yes") {
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${heals[x]}** et guéri \`${healAmount}\` HP, il a maintenant  \`${playerTwoData.battleHealth}\` HP!`
                                )
                                .setColor("GREEN")
                                .setAuthor(
                                    `Heal par: ${nowBattling.displayName}`,
                        ...
                        await channel.send(
                            new MessageEmbed()
                                .setDescription(
                                    `${nowBattling} utilise **${heals[x]}** mais cela a échoué, il a toujours \`${playerTwoData.battleHealth}\` HP!`
                                )
                                .setColor("GREEN")
                                .setAuthor(
                                    `Heal par: ${nowBattling.displayName}`,
        ...
        setTimeout(() => {
            return channel.send(
                new MessageEmbed()
                    ...
                    .setDescription(
                        `${won} a gagné la bataille avec \`${wonData.battleHealth}\` HP restant.!`
                    )
                    .setColor("GREEN")
